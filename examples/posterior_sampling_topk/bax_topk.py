...
for seed in seed_lst[:n_trials]:
    ...
    if save_result:
        # Set up results directory
        # results_dir = Path("topk_results_rebuttal")
        results_dir = Path("examples/posterior_sampling_topk/results_bax")
        results_dir.mkdir(parents=True, exist_ok=True)

    if save_figure:
        # Set up img directory
        img_dir = Path("examples/posterior_sampling_topk/figures_bax")
        img_dir = img_dir /  f'{seed}'
        img_dir.mkdir(parents=True, exist_ok=True)
    ...
    if save_result:
        ...
        np.save(f"{results_dir}/jacc_{seed}.npy", results.expected_metric_jacc_list)
        np.save(f"{results_dir}/norm_{seed}.npy", results.expected_metric_norm_list)
